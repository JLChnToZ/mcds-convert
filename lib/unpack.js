...
async function unpack(input, output, lineWidth, snbt) {
    var _a;
        ...
        if (splittedFile.length > 2 && splittedFile[0] === 'data') {
            ...
            switch (type) {
                case 'structures':
                    if (ext === '.nbt') {
                        if (snbt)
                            result[type][nsid] = await utils_1.nbtToSnbt(zipFile.files[file].async('nodebuffer'), isYaml && lineWidth != null ? lineWidth - 20 : undefined);
                        else
    ...
    await utils_1.writeFileAsync(output, isYaml ? js_yaml_1.dump(result, { lineWidth, condenseFlow: lineWidth == null }) : JSON.stringify(result, null, 2));
}
